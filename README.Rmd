...
Feature           | Description   | Example code  | Example figure
-------------     | ------------- | ------------- | -------------
Piping            | Using the magrittr pipe, `%>%`, you can easily add features to your plot. | [code] | `r print(piping)`
Automatic legend  | For each plot feature, the argument `legend.name` can be included in it's function call. Then, the `legend()` function is used to add the legend to your plot object. | [code]  | `r print(autolegend)`
Error bars        | Apply error bars to points by specifying the upper and lower offsets.  | [code]  | `r print(errorbarsplot)`
Callouts          | Add a line and label in one call.  | [code]  | `r print(calloutsplot)`
Axes reversal     | If the axis needs to be reversed, you can add the argument `reverse = TRUE` to the function call. | [code]  | `r print(axisreversal)`
Automatic limits  | Automatically regenerate limits of plot when new features are added.  | [code]  | `r print(autolimits)`
Embedded functions  | 
