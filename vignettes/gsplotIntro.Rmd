...
demoPlot <- gsplot() %>% 
  ...
  title(main=paste("Site", site), ylab="Flow") %>%
  grid()
demoPlot
...
demoPlot <- gsplot(mar=c(7.1, 4.1, 4.1, 2.1)) %>% 
  lines(dates[[site]], Wtemp[[site]], col="orangered", 
        legend.name="water temperature (F)", ylab='water temp') %>%
  lines(dates[[site]], pH[[site]], col="seagreen", side=4, 
        legend.name="pH (pH Units)", ylab='pH') %>%
...
demoPlot <- gsplot() %>% 
  lines(dates[[site]], Wtemp[[site]], col="orangered") %>% 
  title(main=paste("Site", site), xlab='time', ylab='water temp')
demoPlot
```
